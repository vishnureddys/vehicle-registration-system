...
if(!$wpsc_ticket_id_type){
		$id = 0;
		$id_range = $wpscfunction->get_ticket_id_range();
		do {
				$id = rand($id_range['start_id'], $id_range['end_id']);
...
$assigned_agent = apply_filters( 'wpsc_get_ticket_assigned_agent', $assigned_agent,$args,$ticket_id);

if(!empty($assigned_agent)){
	$wpscfunction->assign_agent( $ticket_id, array_unique($assigned_agent));
}else{
...
$thread_args = array(
  ...
  'customer_name'  => $agent_created ? $agent_name : $customer_name,
  'customer_email' => $agent_created ? $agent_email : $customer_email,
