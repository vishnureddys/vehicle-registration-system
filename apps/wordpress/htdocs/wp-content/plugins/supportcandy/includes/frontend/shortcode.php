...
$wpsc_ct_url = get_option('wpsc_new_ticket_btn_url');
?>

<div class="bootstrap-iso">
  
  <div id="wpsc_tickets_container" class="row" style="background-color:<?php echo $general_appearance['wpsc_bg_color']?> !important;color:<?php echo $general_appearance['wpsc_text_color']?> !important; border-color:<?php echo $general_appearance['wpsc_action_bar_color']?> !important;"></div><?php
  if(strlen($wpsc_ct_url)){?>
    <input type="hidden" id="wpsc_ctu" value="<?php echo $wpsc_ct_url?>"><?php
  }?>
    ...
    <script>
      ...
      <?php
         ...
         foreach ($_GET as $key => $value) {
           $url_attrs[] = '"'.esc_html(sanitize_text_field($key)).'":"'.esc_html(sanitize_text_field($value)).'"';
         }
