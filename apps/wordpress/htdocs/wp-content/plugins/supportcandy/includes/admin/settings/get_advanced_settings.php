...
<form id="wpsc_frm_advanced_settings" method="post" action="javascript:wpsc_set_advanced_settings();">
  <div class="form-group">
    ...
    <div class="form-group" id="wpsc_allow_reply_to_public_tickets" style="<?php echo !$wpsc_ticket_public_mode?'display:none;':'';?>">
		<label for="wpsc_allow_reply_to_public_tickets"><?php _e('Allow others to reply to public tickets','supportcandy');?></label>
		<p class="help-block"><?php _e("Set whether or not users other than raised by can reply to public tickets.","supportcandy");?></p>
		<select class="form-control" name="wpsc_allow_reply_to_public_tickets" id="wpsc_allow_reply_to_public_tickets">
			<?php
			$wpsc_allow_reply_to_public_tickets = get_option('wpsc_allow_reply_to_public_tickets');
			$selected = $wpsc_allow_reply_to_public_tickets == '1' ? 'selected="selected"' : '';
			echo '<option '.$selected.' value="1">'.__('Enable','supportcandy').'</option>';
			$selected = $wpsc_allow_reply_to_public_tickets == '0' ? 'selected="selected"' : '';
			echo '<option '.$selected.' value="0">'.__('Disable','supportcandy').'</option>';
			?>
		</select>
	</div>
	...
	<div class="form-group" id="wpsc_custom_ticket_number_start" style="<?php echo $wpsc_ticket_id_type =='1'? 'display:block': 'display:none'?>">
	...
	<?php $wpsc_rt_id_length = get_option('wpsc_rt_id_length'); ?>
	<div class="form-group" id="wpsc_rt_id_length_set" style="<?php echo $wpsc_ticket_id_type == '0'? 'display:block':'display:none'?>">
		<label for="wpsc_rt_id_length"><?php _e('Random id length','supportcandy');?></label>
		<p class="help-block"><?php _e("Number of digits for random ticket id.","supportcandy");?></p>
		<div class="row">
			<input type="number" min="1" class="form-control" name="wpsc_rt_id_length" id="wpsc_rt_id_length" value="<?php echo htmlentities($wpsc_rt_id_length) ?>"/>
		</div>
	</div>
	...
	<div class="form-group">
		<label for="wpsc_auto_delete"><?php _e('Auto delete closed tickets','supportcandy');?></label>
		<p class="help-block"><?php _e("If enabled, closed tickets will get deleted after given time interval.","supportcandy");?></p>
	...
	<div class="form-group">
		<label for="wpsc_new_ticket_btn_url"><?php _e('New ticket button url','supportcandy');?></label>
		<p class="help-block"><?php _e("Add create ticket page url if you do not want to use SupportCandy create ticket form.","supportcandy");?></p>
		<input class="form-control" type="text" id="wpsc_new_ticket_btn_url" name="wpsc_new_ticket_btn_url" value="<?php echo get_option('wpsc_new_ticket_btn_url')?>">
	</div>
...
<script>
jQuery(document).ready(function() {	   
   jQuery('#wpsc_ticket_public_mode').change(function() {
		 if(this.value=='1'){			 
			  jQuery('#wpsc_allow_reply_to_public_tickets').show();
		 }else {
		 		jQuery('#wpsc_allow_reply_to_public_tickets').hide(); 
		 }
	});

	jQuery('#wpsc_ticket_id_type').change(function() {
		if(this.value=='1'){			 
			jQuery('#wpsc_custom_ticket_number_start').show();
			jQuery('#wpsc_rt_id_length_set').hide();
		}else {
			jQuery('#wpsc_custom_ticket_number_start').hide();
			jQuery('#wpsc_rt_id_length_set').show(); 
		}
	});
});
</script>