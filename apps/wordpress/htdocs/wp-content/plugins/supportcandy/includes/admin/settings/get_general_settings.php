...
<form id="wpsc_frm_general_settings" method="post" action="javascript:wpsc_set_general_settings();">
    ...
    <div class="form-group">
      ...
      <select class="form-control" name="wpsc_calender_date_format" id="wpsc_calender_date_format">
		...
		<div class="form-group">
			<label for="wpsc_allow_ct"><?php _e('Allow to create new tickets','supportcandy');?></label>
			<p class="help-block"><?php _e("Only selected roles can create new tickets. Enables guest ticket facility. Guest will able to create ticket without needing them to have an user account on website. They can create ticket using their name and email address.","supportcandy");?></p>
			<div class="row"><?php
				$wpsc_allow_to_create_ticket = get_option('wpsc_allow_to_create_ticket');
				$wpsc_allow_to_create_ticket = $wpsc_allow_to_create_ticket ? $wpsc_allow_to_create_ticket : array();
				foreach($agent_role as $key => $val):
					$checked = in_array($key,$wpsc_allow_to_create_ticket) ? 'checked="checked"' : '';?>
					<div class="col-sm-4" style="margin-bottom:10px; display:flex;">
						<div style="width:25px;"><input type="checkbox" name="wpsc_allow_to_create_ticket[]" <?php echo $checked?> value="<?php echo $key?>" /></div>
						<div style="padding-top:3px;"><?php echo $val['label']?></div>
					</div>
					<?php
				endforeach;
				$ct_customer = in_array('customer',$wpsc_allow_to_create_ticket) ? 'checked="checked"' : '';
				$ct_guest = in_array('guest',$wpsc_allow_to_create_ticket) ? 'checked="checked"' : '';
				?>
				<div class="col-sm-4" style="margin-bottom:10px; display:flex;">
					<div style="width:25px;"><input type="checkbox" name="wpsc_allow_to_create_ticket[]" <?php echo $ct_customer?> value="customer" /></div>
					<div style="padding-top:3px;"><?php _e('Customer','supportcandy')?></div>
                </div>
				<div class="col-sm-4" style="margin-bottom:10px; display:flex;">
					<div style="width:25px;"><input type="checkbox" name="wpsc_allow_to_create_ticket[]" <?php echo $ct_guest?> value="guest" /></div>
		...
		<div class="form-group">
			...
			<p class="help-block"><?php _e("Only selected roles can reply to closed tickets. The tickets having status selected in the closed ticket group will be considered as close tickets.","supportcandy");?></p>
			<div class="row">
				<div class="col-sm-4" style="margin-bottom:10px; display:flex;">
