...
$ticket_url = get_permalink(get_option('wpsc_support_page_id')).'?support_page=open_ticket&ticket_id='.$ticket_id.'&auth_code='.$auth_id;

$after_close_redirect = apply_filters('wpsc_after_close_redirect', true, $ticket);
?>
...
<div class="row wpsc_tl_action_bar" style="background-color:<?php echo $general_appearance['wpsc_action_bar_color']?> !important;">
	...
	<div class="col-sm-12">
		...
		<?php endif;?>
		<?php 
			$allow_ug_member_ct = apply_filters('wpsc_allow_close_ticket', false, $ticket);
			if ( ($customer_email == $current_user->user_email && get_option('wpsc_allow_customer_close_ticket')) || ($wpscfunction->has_permission('change_status',$ticket_id)) || $allow_ug_member_ct ):
     ...
     <?php
		...
		<button type="button" id="wpsc_individual_copyurl_btn" onclick="wpsc_copy_ticket_url()" class="btn btn-sm wpsc_action_btn" style="<?php echo $action_default_btn_css?>"><i class="far fa-clipboard"></i> <?php _e('Copy URL','supportcandy')?></button>
		<div style="display:none" id="wpsc_ticket_url"><?php echo $ticket_url ?></div>
  </div>
      ...
      <h4>
		...
		<div class="row wpsc_threads_container">
			...
			foreach ($threads as $thread):
				$reply = stripslashes(addslashes(htmlspecialchars_decode($thread->post_content, ENT_QUOTES)));
								...
								<div class="row"  id="wpsc_raised_by_widget" style="background-color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_ticket_widgets_bg_color']?> !important;color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_ticket_widgets_text_color']?> !important;border-color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_ticket_widgets_border_color']?> !important;">
							      <h4 class="widget_header"><i class="fas fa-user-tie"></i> <?php echo $ticket_widget_name;?>
										...
										<div class="wpsp_sidebar_labels" id="wpsc_user_add_info" style="font-size:20px; cursor:pointer;color:#a7a9ab;">
											...
											<?php do_action('wpsc_add_info_icon_raised_by', $ticket); ?>
										</div>
									<?php } ?>
									<?php do_action('wpsc_raised_by_widget_content');?>
							    </div>
								<?php
					...
					if ($ticket_widget->slug=="additional-recepients" && $flag):
						?>
							<div class="row"  id="wpsc_add_people_widget" style="background-color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_ticket_widgets_bg_color']?> !important;color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_ticket_widgets_text_color']?> !important;border-color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_ticket_widgets_border_color']?> !important;">
							      <h4 class="widget_header wpsc_add_recipients"><i class="fas fa-envelope"></i> <?php echo $ticket_widget_name;?>
...
<script>
	...
	function wpsc_post_note(description){
        ...
        if($wpsc_reply_bcc){
		?>
			var bcc_emails = jQuery('#reply_bcc').val().trim();
			if (bcc_emails) {
				var emails = bcc_emails.split(",");
	...
	function wpsc_copy_ticket_url(){
		var temp = jQuery("<input>");
		jQuery("body").append(temp);
		temp.val(jQuery('#wpsc_ticket_url').text()).select();
		document.execCommand("copy");
		temp.remove();

		jQuery('#wpsc_alert_success .wpsc_alert_text').text('Ticket URL copied!');
		jQuery('#wpsc_alert_success').slideDown('fast',function(){});
    	setTimeout(function(){ jQuery('#wpsc_alert_success').slideUp('fast',function(){}); }, 1000);
	}
	...
	function wpsc_set_close_ticket(ticket_id){
		...
		.done(function (response_str) {<?php 
			if($after_close_redirect){?>
				wpsc_get_ticket_list();<?php
			}else{?>
				wpsc_open_ticket(ticket_id);<?php
			}
			?>
		});
	}
