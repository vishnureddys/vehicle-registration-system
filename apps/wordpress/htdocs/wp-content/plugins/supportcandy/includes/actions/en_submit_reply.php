...
foreach ($email_templates as $email) :
  ...
  if( $wpscfunction->check_ticket_conditions($conditions,$ticket_id) ) :
    ...
    if(in_array('current_user', $recipients)){
      $email_addresses[] = $last_reply['user_email'];
    }else{
    ...
    $to = apply_filters('wpsc_submit_reply_to_email',$to,$email_addresses,$ticket_id);
    $bcc = apply_filters('wpsc_submit_reply_bcc_emails',$bcc,$email_addresses,$ticket_id);
